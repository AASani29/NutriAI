  ...
  return (
    <section id="impact" className="relative py-24 bg-black text-white overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20">
        ...
        <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-secondary rounded-full blur-[120px] -ml-20 -mb-20"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <h2 className="text-4xl md:text-5xl font-display font-bold leading-tight mb-6">
            ...
            <motion.span transition={{duration:0.6, ease:'easeIn'}} whileInView={{opacity:1, x:0}} initial={{x:20, opacity:0}}  viewport={{once:true}} className="block ml-16 text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div transition={{ duration: 0.5, ease: 'easeIn', delay: stat.delay }} whileInView={{ opacity: 1, x: 0 }} initial={{ x: stat.xpos, opacity: 0 }} viewport={{ once: true }} key={index} className="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-3xl hover:bg-white/10 transition-colors duration-300 flex flex-col items-center text-center group">
                <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                  <Icon className="w-6 h-6 text-primary group-hover:text-white transition-colors" />
                </div>
                <div className="text-4xl font-bold mb-2">{stat.number}</div>
                <div className="font-semibold text-secondary mb-3">{stat.label}</div>
